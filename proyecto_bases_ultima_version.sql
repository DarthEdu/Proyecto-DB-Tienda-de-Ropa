...
--  @Bloqueo exclusivo (X-lock)
START TRANSACTION;

-- Adquiere un bloqueo exclusivo en la fila del producto con codigo_producto = 1
UPDATE Productos SET stock = stock - 1 WHERE codigo_producto = 1;

-- @Bloqueo compartido (S-Lock)

START TRANSACTION;

-- Adquiere un bloqueo compartido en la fila del producto con codigo_producto = 1
SELECT * FROM Productos WHERE codigo_producto = 1 LOCK IN SHARE MODE;
...
-- Bloquea la tabla 'Clientes' para operaciones de lectura
LOCK TABLES Clientes READ;

-- Realiza alguna operación de lectura
...
-- Bloquea la tabla 'Pedido' para operaciones de escritura
LOCK TABLES Pedido WRITE;

-- Realiza alguna operación de escritura
UPDATE Pedido SET cantidad = cantidad + 1 WHERE id_pedido = 1;
...
-- Bloqueo explícito de la tabla 'Clientes' para escritura
LOCK TABLES Clientes WRITE;

-- Ahora realizas la operación de inserción
INSERT INTO Clientes (CI, nombre, correo, numero_telefono, direccion) 
VALUES (12345678, 'Ana López', 'ana@example.com', '5551234', 'Calle 123');

-- Los bloqueos se mantienen hasta que se haga COMMIT o ROLLBACK
COMMIT;
...
-- Este comando automáticamente aplica un bloqueo exclusivo en la fila donde codigo_producto = 1
UPDATE Productos SET stock = stock - 1 WHERE codigo_producto = 1;

-- Si se intenta hacer un SELECT desde otra transacción en esta fila, se bloqueará hasta que se haga COMMIT o ROLLBACK.


-- ##### RESOLUCION DE BLOQUEOS

-- @ TIMEOUTS DE BLOQUEOS

SET GLOBAL innodb_lock_wait_timeout = 10;  -- Establece el timeout a 10 segundos
...
UPDATE Clientes SET nombre = 'Juan Pérez' WHERE CI = 12345678;

-- Verifica alguna condición (por ejemplo, que la fila haya sido actualizada)
...
-- #### FENOMENOS DE CONCURRENCIA

-- @ LECTURA SUCIA
-- Transacción 1: Cambia el nivel de aislamiento a Read Uncommitted
SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
START TRANSACTION;

-- Actualiza el stock de un producto, pero aún no hace COMMIT
UPDATE Productos SET stock = stock - 1 WHERE codigo_producto = 1;

-- Transacción 2: Lee el valor no confirmado de la Transacción 1
SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;
START TRANSACTION;

-- Lectura sucia
SELECT stock FROM Productos WHERE codigo_producto = 1;

-- Si Transacción 1 hace ROLLBACK, Transacción 2 habría leído un valor incorrecto

-- @Lectura NO REPETIBLE

-- Transacción 1: Cambia el nivel de aislamiento a Read Committed
SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED;
START TRANSACTION;

-- Lee el valor del stock
SELECT stock FROM Productos WHERE codigo_producto = 1;

-- Transacción 2: Actualiza el stock y confirma los cambios
SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED;
START TRANSACTION;
...
-- Transacción 1: Vuelve a leer el mismo valor de stock
SELECT stock FROM Productos WHERE codigo_producto = 1;

-- Los valores leídos en las dos consultas pueden ser diferentes, lo que se llama lectura no repetible.

-- @ FANTASMAS

-- Transacción 1: Cambia el nivel de aislamiento a Repeatable Read
SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ;
START TRANSACTION;

-- Lee todos los productos con stock mayor a 10
SELECT * FROM Productos WHERE stock > 10;

-- Transacción 2: Inserta un nuevo producto con stock mayor a 10 y confirma los cambios
SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ;
START TRANSACTION;

INSERT INTO Productos (codigo_producto, nombre, stock) VALUES (10, 'Nuevo Producto', 20);
COMMIT;

-- Transacción 1: Vuelve a ejecutar la misma consulta
SELECT * FROM Productos WHERE stock > 10;

-- La nueva fila no aparecerá en la segunda consulta de Transacción 1 en Repeatable Read
-- Si el nivel de aislamiento fuera Read Committed, la nueva fila sí aparecería.
...
a)	Ejecución de procesos por lotes: programación de tareas, manejo de errores en lotes.
b)	Funciones definidas por el usuario: creación, uso en consultas, optimización.*/
...
BEGIN
  DECLARE i INT DEFAULT 0;
  WHILE i < 10 DO
    INSERT INTO clientes (nombre, correo) VALUES (CONCAT('Cliente ', i), CONCAT('cliente', i, '@ejemplo.com'));
    SET i = i + 1;
  END WHILE;
END//
DELIMITER ;
...
CREATE DEFINER = root@localhost function hola_mundo(nombre varchar(30))
returns varchar(35) 
DETERMINISTIC
return concat('hola', nombre);


select hola_mundo(nombre) from clientes;


CREATE DEFINER= root@localhost FUNCTION fecha_invalida(fecha varchar(30))
RETURNS varchar(35) 
DETERMINISTIC return
 'fecha_invalida.so';

SELECT fecha_invalida('2023-02-30');

...
CREATE TABLE Productos(
    ...
    precio_producto DOUBLE(7,2) NOT NULL,  -- Increased size to 7,2 for a broader price range
    stock INT NOT NULL,
...
CREATE TABLE Pedido(
    ...
    valor_pedido DOUBLE(7,2) NOT NULL,  -- Increased size to 7,2 for larger order values
...
CREATE TABLE Categorias(
    ...
    codigo_producto INT,
    FOREIGN KEY (codigo_producto) REFERENCES Productos(codigo_producto)
    ON DELETE SET NULL  -- Setting to NULL instead of CASCADE to avoid unintentional deletions
    ON UPDATE CASCADE
);
